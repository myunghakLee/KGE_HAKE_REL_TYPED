...
class DataReader(object):
    def __init__(self, data_path: str):
        ...
        type_dict_path = os.path.join(data_path, 'typed.dict')
        ...
        self.type_dict = self.read_dict(type_dict_path)

        self.train_data = self.read_data(train_data_path, self.entity_dict, self.relation_dict, self.type_dict)
        self.valid_data = self.read_data(valid_data_path, self.entity_dict, self.relation_dict, self.type_dict)
        self.test_data = self.read_data(test_data_path, self.entity_dict, self.relation_dict, self.type_dict)
    ...
    def read_data(self, data_path: str, entity_dict: Dict[str, int], relation_dict: Dict[str, int], type_dict: Dict[str, int]):
        """
        ...
        with open(data_path, 'r') as f:
            for line in f:
                head, head_type, relation, tail, tail_type = line.strip().split('\t')
                triples.append((entity_dict[head], type_dict[head_type], relation_dict[relation], \
                                entity_dict[tail], type_dict[tail_type]))
        return triples
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        head, head_type, rel, tail, tail_type = pos_triple
        ...
        while neg_size < self.neg_size:
            ...
            if self.batch_type != BatchType.REL_BATCH:
                neg_triples_tmp_type = np.array([self.entity2type_dict[t] for t in neg_triples_tmp])
            else:
                neg_triples_tmp_type = np.array([-1.0])# debuging용, 원래 있으면 안됨

            neg_triples.append(neg_triples_tmp)
            neg_triples_type.append(neg_triples_tmp_type)

            neg_size += neg_triples_tmp.size

        neg_triples_type = np.concatenate(neg_triples_type)[:self.neg_size]
        neg_triples_type = torch.from_numpy(neg_triples_type)
        ...
        return pos_triple, neg_triples, neg_triples_type, subsampling_weight, self.batch_type

    @staticmethod
    def collate_fn(data):
        positive_sample = torch.stack([_[0] for _ in data], dim=0)
        negative_sample = torch.stack([_[1] for _ in data], dim=0)
        negative_sample_type = torch.stack([_[2] for _ in data], dim=0)
        subsample_weight = torch.cat([_[3] for _ in data], dim=0)
        batch_type = data[0][4]
        return positive_sample, negative_sample, negative_sample_type, subsample_weight, batch_type
    ...
    def two_tuple_count(self):
        ...
        init_cnt = 3
        for head,head_type, rel, tail,tail_type in self.triples:
...
#             negative_sample_type = torch.tensor([self.entity2type_dict[d.item()] for d in negative_sample])
    ...
    def collate_fn(data):
        ...
        mode = data[0][4]
        return positive_sample, negative_sample, negative_sample_type, filter_bias, mode
